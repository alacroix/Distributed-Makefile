...
Manager::Manager(std::string master_rule, std::map<std::string, Rule *> dictionary, QueueDoable *queueDoable, int nbrRules)
        :
...
void Manager::execute() {
    ...
    std::cout << "Nombre rules = " << nbrRules << std::endl;
    std::string nomRegle;

#pragma omp parallel private(nomRegle)
    {
...
//#pragma omp critical
            std::cout << printCurrentThread() << "executing " << nomRegle << std::endl;
            std::cout << " adresse queue : " << &queueDoable->getQueueDoable()[0] << std::endl;
